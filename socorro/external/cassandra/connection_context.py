...
class Connection(object):
    """A facade in front of a Cassandra connection that standardizes certain gross
    ...
    This class is as thread safe as the underlying connection - as of 2014,
    that means not thread safe.
    """

    #--------------------------------------------------------------------------
    def __init__(self, config, connection, keyspace='CrashData', column_family='CrashInfoCounter'):
        """Construct.

        parameters:
            config - a mapping containing hostname, port and other typical connection info
            connection - A Cassandra BlockingConnection object
        """
        self.config = config
        self.keyspace = keyspace
        self.connection = pycassa.ColumnFamily(connection, column_family)
...
class ConnectionContext(RequiredConfig):
    ...
    to Cassandra wrapped in the minimal Connection class above.  Suitable for
    use in a "with" statment this class will handle opening a connection to
    Cassandra and its subsequent closure.  Use this class only when connections
    ...
    # to a Cassandra server.
    required_config = Namespace()
    required_config.add_option(
        name='host',
        default='localhost',
        doc='the hostname of the Cassandra server',
        reference_value_from='resource.cassandra',
    )
    required_config.add_option(
        name='port',
        default='9160',
        doc='the port the Cassandra server is listening on',
        reference_value_from='resource.cassandra',
    )
    required_config.add_option(
        name='cassandra_user',
        default=None,
        doc='the name of the user within the Cassandra instance',
        reference_value_from='secrets.cassandra',
    )
    required_config.add_option(
        name='cassandra_password',
        default=None,
        doc="the user's Cassandra password",
        reference_value_from='secrets.cassandra',
    )
    required_config.add_option(
        name='keyspace',
        default='CrashData',
        doc="keyspace in Cassandra we'd like this connection to associate with",
        reference_value_from='resource.cassandra',
    )
    required_config.add_option(
        name='column_family',
        default='CrashInfoCounter',
        doc="column_family we're putting data into",
        reference_value_from='resource.cassandra',
    )
    required_config.add_option(
        name='cassandra_connection_wrapper_class',
        default=Connection,
        doc="a classname for the type of wrapper for Cassandra connections",
        reference_value_from='resource.cassandra',
    )
    required_config.add_option(
        name='timeout',
        default=5,
        doc="timeout for connections in seconds",
        reference_value_from='resource.cassandra',
    )
    required_config.add_option(
        name='pool_size',
        default=5,
        doc="default size of connection pool",
        reference_value_from='resource.cassandra',
    )

    #--------------------------------------------------------------------------
    def __init__(self, config, local_config=None):
        """Initialize the parts needed to start making Cassandra connections
        ...
        parameters:
            ...
            local_config - this is the namespace within the complete config
                           where the actual Cassandra parameters are found"""
        ...
        # For now.. considering everything not retriable due to possibility of
        # duplicate transactions without errors raised by Cassandra
        self.operational_exceptions = (
          pycassa.AuthenticationException,
          pycassa.AuthorizationException,
          pycassa.InvalidRequestException,
    ...
    def connection(self, name=None):
        """create a new Cassandra connection, set it up for our queues, then
        return it wrapped with our connection class.

        parameters:
            name - unused in this context
        """
        server = ':'.join([self.local_config.host, self.local_config.port])
        bare_cassandra_pool = pycassa.pool.ConnectionPool(
            self.local_config.keyspace,
            server_list=[server],
            #credentials={
                #'username': self.local_config.cassandra_user,
                #'password': self.local_config.cassandra_password
            #},
            timeout=self.local_config.timeout,
            pool_size=self.local_config.pool_size
        )

        wrapped_connection = \
            self.local_config.cassandra_connection_wrapper_class(
                self.config,
                bare_cassandra_pool,
                self.local_config.keyspace
            )
    ...
    def __call__(self, name=None):
        """returns a Cassandra connection wrapped in a contextmanager.
        ...
        parameters:
            name - an optional name for the Cassandra connection"""
        wrapped_connection = self.connection(name)
        try:
            yield wrapped_connection
        finally:
            self.close_connection(wrapped_connection)
    ...
    def close_connection(self, connection, force=False):
        ...
        parameters:
            connection - the Cassandra connection object
...
class ConnectionContextPooled(ConnectionContext):
    """A factory object in the form of a functor.  It returns connections
    to Cassandra wrapped in the minimal Connection class above.  It implements
    ...
    by this functor.  This ensures thread safety for Cassandra's unsafe
    connections.
    """
    #--------------------------------------------------------------------------
    def __init__(self, config, local_config=None):
    ...
    def connection(self, name=None):
        ...
        if name in self.pool:
            #self.config.logger.debug('fetching Cassandra connection: %s', name)
            return self.pool[name]
        self.config.logger.debug('creating new Cassandra connection: %s', name)
        self.pool[name] = \
    ...
    def close_connection(self, connection, force=False):
        ...
        if force:
            try:
                ...
                self.config.logger.error('CassandraPooled - failed closing')
